...
#define KMS(a) klee_make_symbolic(&a, sizeof(a), #a)
    KMS(Cur_Vertical_Sep);
    KMS(High_Confidence);
    KMS(Two_of_Three_Reports_Valid);
    KMS(Own_Tracked_Alt);
    KMS(Own_Tracked_Alt_Rate);
    KMS(Other_Tracked_Alt);
    KMS(Alt_Layer_Value);
    KMS(Up_Separation);
    KMS(Down_Separation);
    KMS(Other_RAC);
    KMS(Other_Capability);
    KMS(Climb_Inhibit);
    klee_assume((Alt_Layer_Value >= 0) & (Alt_Layer_Value < 4));
    fprintf(stdout, "%d\n", alt_sep_test());
    exit(0);
}
