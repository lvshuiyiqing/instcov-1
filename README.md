...
6.  When you have run one or more test cases, and have obtained one or more
	trace files, you can analysis the MC/DC coverage using `mcdc-scan`:
	...
	Additionally, `mcdc-scan` also supports encoding the problem of minimizing
...
* `instcov-view.py`: it is just a wrapper for instcov-view.
...
You need to run several different test cases, and call `mcdc-scan` on the
